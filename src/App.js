...
export default function App() {
  const { phonenumber } = useParams(); // ✅ Only load data for this phone number
  const [invoices, setInvoices] = useState([]);
  const [editId, setEditId] = useState(null);
  const [editData, setEditData] = useState({});
  const [loading, setLoading] = useState(false);

  /** ---------------- Fetch data (only for user) ---------------- */
  const fetchData = async () => {
    if (!phonenumber) return;
    const { data, error } = await supabase
      .from("backend")
      .select("*")
      .eq("phonenumber", phonenumber); // ✅ Filter by user phone number
  ...
  return (
    <div style={{ padding: 20, opacity: loading ? 0.6 : 1 }}>
      <h2 style={{ marginBottom: 12 }}>
        Invoices for {phonenumber ? `+${phonenumber}` : "User"}
      </h2>
